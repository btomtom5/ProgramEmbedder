...
def matrix_replicates(matrices):
    ...
    return tf.transpose(
        tf.reshape(
                ...
                [-1,  H1_UNITS**2, MAX_SEQUENCE_LENGTH,]),
        [0, 2, 1])
...
with tf.Session() as sess:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        while True:
            try:
                _, train_loss_val = sess.run([optimizer, loss], feed_dict={
                    ...
                    Mats: sess.run(matrix_replicates(matrices_train))
                })
                print('Epoch {}: Minibatch Loss: {}'.format(epoch, train_loss_val))
            except tf.errors.OutOfRangeError:
                break
            except tf.errors.InvalidArgumentError:
                raise  # typically happens when there isn't enough data for a given AST
        ...
        eval_loss_val = sess.run([loss], feed_dict={
            ...
            Mats: sess.run(matrix_replicates(tf.expand_dims(matrices_eval, axis=0)))
        })
    ...
    test_loss_val = sess.run([loss], feed_dict={
        ...
        Mats: sess.run(matrix_replicates(tf.expand_dims(matrices_test, axis=0)))
    })
